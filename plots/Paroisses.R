output$paroissesNb <- renderPlot({
  ...
  nombre_paroisses <- JIAP_paroisses %>% group_by(Annee, seed) %>% summarise(nb = n())
...
output$paroissesNbFilter <- renderPlot({
  nombre_paroisses <- filtred$paroisses %>% group_by(Annee, seed) %>% summarise(nb = n())
  ggplot(nombre_paroisses, aes(factor(Annee), nb)) +
    geom_tufteboxplot() +
    xlab("Temps") + ylab("Nombre de paroisses") +
    ggtitle("Évolution du nombre de paroisses")
...
output$paroissesCompo <- renderPlot({
  ...
  paroisses_breaks <- JIAP_paroisses %>%
    filter(Annee %in% c(820, 940, 1040, 1160)) %>%
    mutate(NbFidelesBreaks = cut(NbParoissiens, breaks = fidelesBreaks, labels = fidelesLabels)) %>%
    group_by(seed, Annee, NbFidelesBreaks) %>%
    summarise(NbParoisses = n())
  
  ggplot(paroisses_breaks, aes(factor(NbFidelesBreaks), NbParoisses)) +
    geom_tufteboxplot() + facet_wrap(~Annee, scale="free") +
...
output$paroissesSuperficie <- renderPlot({
  ...
  paroisses_sup_breaks <-  JIAP_paroisses %>%
    filter(Annee %in% c(820, 940, 1040, 1160)) %>%
    mutate(NbSuperficiesBreaks = cut(Superficie, breaks = superficieBreaks, labels = superficieLabels)) %>%
