...
currExperiment <- "base2_compo5_2_6"
...
```{r reactive base datasets, include = FALSE}
goodSeeds <- reactive({
  fullFP_raw %>%
    group_by(Seed) %>%
...
parameters_data <- reactive({
  nonUniqueParams <- parameters_raw %>%
    gather(key = "Var", value = "Value") %>%
    group_by(Var, Value) %>%
    mutate(Freq = n()) %>%
    filter(Freq != nrow(parameters_raw)) %>%
    extract2("Var") %>%
    unique(.)
  
  parameters_raw %>% select_( .dots = nonUniqueParams)
  })
...
if (length(input$parcoords_brushed_row_names) > 0){
  goodSeeds()[input$parcoords_brushed_row_names,]
} else {
  goodSeeds()
}
  })
  
global_subset <- reactive(global_data() %>% filter(Seed %in% filtredSeeds())) 
seigneurs_subset <- reactive(seigneurs_data() %>% filter(Seed %in% filtredSeeds())) 
agregats_subset <- reactive(agregats_data() %>% filter(Seed %in% filtredSeeds())) 
poles_subset <- reactive(poles_data() %>% filter(Seed %in% filtredSeeds())) 
paroisses_subset <- reactive(paroisses_data()%>% filter(Seed %in% filtredSeeds())) 
fullFP_subset <- reactive(fullFP_data() %>% filter(Seed %in% filtredSeeds())) 
summFP_subset <- reactive(summFP_data() %>% filter(Seed %in% filtredSeeds()))  

paroisses_geom <- reactive({
  setDT(paroisses_data())[,strsplit(gsub('\\[{|}\\]','', Geom, perl=T), '}, *{', perl=T), .(Seed, Annee, IdParoisse)] %>%
...
```{r parameters input}
fillCol(height = 300, 
  parcoordsOutput("parcoords", height = "100%")
)

output$parcoords <- renderParcoords({
    parcoords(parameters_data() %>% select(-myseed),
            rownames=FALSE,
            brushMode="1d",
            reorderable = TRUE)
})
...
```{r summary table2, include = FALSE}
...
```
> Toutes les simulation


### Chart 2

```{r print summary select}
renderFormattable({
...
```{r}
renderDataTable(paroisses_geom())
```
